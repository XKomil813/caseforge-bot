...
function startRoulette(wonSkin, itemsPool, openBtn, statusDisplay) {
    ...
    try {
        ...
        for (let i = 0; i < totalItems; i++) {
            ...
            const color = safeItem.price >= 80000 ? '#ffd700' : safeItem.price >= 25000 ? '#eb4b4b' : safeItem.price >= 8000 ? '#d32ee6' : safeItem.price >= 1500 ? '#4b69ff' : '#b0c3d9';
...
function renderInventory() {
    ...
    inventoryList.innerHTML = userInventory.map((item, index) => {
        ...
        const borderColor = safeItem.price >= 25000 ? 'border-red-500/30' : 
                           safeItem.price >= 8000 ? 'border-purple-500/30' : 
                           safeItem.price >= 1500 ? 'border-blue-500/30' : 'border-white/10';
